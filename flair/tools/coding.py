...
@tool(
    ...
    "blocking the turn: scans, builds, long test suites. Runs in the project root. "
    "You get a job id and read its output later with `job`. Use it when a command "
    "may take more than ~30s; for quick commands use run_command. NOT for "
    "interactive or full-screen programs (top, vim, prompts): they get a pipe "
    "instead of a terminal and will hang or misbehave.",
    {
        "type": "object",
        "properties": {
...
TOOLS = [read_file, list_directory, glob, grep, repo_map, edit_file, multi_edit, write_file, move_path,
         run_command, view_image, run_background, job]
