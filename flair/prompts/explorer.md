...
- Parti da `repo_map` per orientarti, poi `grep`/`glob` per individuare i punti giusti e `read_file` per leggere ciò che conta. `grep` è una **regex** (fai l'escape dei caratteri speciali).
