...
- **Coding agent** — `read_file`, `list_directory`, `glob`, `grep`, `repo_map`, `edit_file`, `multi_edit`, `write_file`, `run_command`, `explore`, `plan`, plus read-only `web_search` / `web_fetch` for information that lives online (library docs, API signatures, error messages). File tools are **sandboxed** to the project root (`--root`): it cannot escape it.
...
Offline suite (no network, fake provider) with ~446 assertions covering: robust argument parsing, usage normalization for both providers, the **real provider request path** (parameters sent to the API: `max_tokens` vs `max_completion_tokens`, `temperature` omitted on reasoning models, DeepSeek V4 thinking enabled via parameter, retry on transient errors only), **streaming assembly**, **compaction** and overflow recovery, the resilient `edit_file` matcher and **atomic `multi_edit`**, **actionable missing-argument errors** (naming the missing arg and suggesting the intended one, e.g. `filename`→`path`), the **`repo_map`** outline across ~two dozen languages, the **read-only `explore` sub-agent** (isolation, read-only toolset, no recursion, leak-proof usage roll-up), the **`plan`** tool and **stage-0 context pruning** (rules, guarantees, summary-skip), web **search** (multi-backend cascade + errors) and **fetch**, **session persistence** (save/resume round-trip and **atomic writes**, at both the store and CLI level), **runtime provider/model switching**, the context indicator, the router (including deterministic continuation stickiness), **headless execution** (significant exit codes, the `--json` result object, read-only tool filtering, the hard cost budget), and **both** agents on the real tools.
